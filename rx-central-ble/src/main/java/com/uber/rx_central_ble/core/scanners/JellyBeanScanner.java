...
public class JellyBeanScanner implements Scanner {
  ...
  public JellyBeanScanner(AdvertisingData.Factory scanDataFactory) {
  ...
  private BluetoothAdapter.LeScanCallback getScanCallback() {
    return (bluetoothDevice, rssi, eirData) -> {
      if (scanMatcher != null && scanDataSubject != null) {
        AdvertisingData advertisingData = scanDataFactory.produce(bluetoothDevice, eirData, rssi);
        if (scanMatcher.match(advertisingData)) {
